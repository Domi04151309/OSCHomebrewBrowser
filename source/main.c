...
int main(int argc, char **argv) {
	...
	while(1) {
		...
		UI_roundedRect(UI_PAGE_X, UI_PAGE_Y, UI_PAGE_W_LARGE, UI_PAGE_H, COLOUR_WHITE);
		...
		if (ACTIVITIES_current() == ACTIVITY_MAIN) {
			UI_roundedRect(UI_PAGE_X, UI_PAGE_Y, UI_PAGE_W_LARGE, UI_PAGE_H, COLOUR_LIGHT_GRAY);
			UI_roundedRect(UI_PAGE_X, UI_PAGE_Y, UI_PAGE_W, UI_PAGE_H, COLOUR_WHITE);
			...
			// GC scrolling
			if ((strlen(homebrew_list[finish+1].name) > 1 && select_repo == false) || (select_repo == true && finish_updated+1 < repo_count)) {
			...
			if ((select_repo == false && ypos <= 140) || (select_repo == true && ypos <= 180)) {
				if (PAD_SubStickY(0) > 18) {
					ypos+= 2;
				}
				if (PAD_SubStickY(0) > 28) {
			...
			GRRLIB_ClipDrawing(UI_PAGE_X, UI_PAGE_Y, UI_PAGE_W, UI_PAGE_H);
			int b;
			for(b = 0; b <= 4; b++) {
				if (strlen(homebrew_list[(start + b)].name) >= 2 && strcmp(homebrew_list[(start + b)].name,"000") != 0) {
					if (UI_isOnSquare(ir, UI_PAGE_X, ypos + (76 * (start + b)), 530, 64)) {
						doRumble = true;
						UI_highlight(UI_PAGE_X, ypos + (76 * (start + b)), UI_PAGE_W, 64);
						...
						if ((pressed & WPAD_BUTTON_PLUS || pressed_gc & PAD_TRIGGER_R) && homebrew_list[(start + b)].local_app_size > 0 && homebrew_list[(start + b)].in_download_queue == 2) {
							homebrew_list[(start + b)].in_download_queue = false;
						}
						else if ((pressed & WPAD_BUTTON_PLUS || pressed_gc & PAD_TRIGGER_R) && homebrew_list[(start + b)].local_app_size != homebrew_list[(start + b)].app_size) {
							homebrew_list[(start + b)].in_download_queue = true;
						}
						else if ((pressed & WPAD_BUTTON_MINUS || pressed_gc & PAD_TRIGGER_L) && homebrew_list[(start + b)].local_app_size != homebrew_list[(start + b)].app_size && homebrew_list[(start + b)].in_download_queue == true) {
							homebrew_list[(start + b)].in_download_queue = false;
						}
						else if ((pressed & WPAD_BUTTON_MINUS || pressed_gc & PAD_TRIGGER_L) && homebrew_list[(start + b)].local_app_size > 0) {
			...
			//Sorting icons
			if (UI_isOnImg(ir, UI_SORT_X, 150, name_img)) {
				doRumble = true;
				GRRLIB_DrawImg(UI_SORT_X, 150, name_img, 0, 1, 1, LIGHT_BTN_COLOR_HOVER);
				if (setting_tool_tip == true) { UI_drawTooltip(UI_SORT_X, 150, STR_SORT_BY_NAME); }
				if (pressed & WPAD_BUTTON_A || pressed & WPAD_BUTTON_2 || pressed_gc & PAD_BUTTON_A) {
					// Free list strings
					free_string = true;
				...
				GRRLIB_DrawImg(UI_SORT_X, 150, name_img, 0, 1, 1, LIGHT_BTN_COLOR);
			}
			if (UI_isOnImg(ir, UI_SORT_X, 200, date_img)) {
				doRumble = true;
				GRRLIB_DrawImg(UI_SORT_X, 200, date_img, 0, 1, 1, LIGHT_BTN_COLOR_HOVER);
				if (setting_tool_tip == true) { UI_drawTooltip(UI_SORT_X, 200, STR_SORT_BY_DATE); }
				if (pressed & WPAD_BUTTON_A || pressed & WPAD_BUTTON_2 || pressed_gc & PAD_BUTTON_A) {
					// Free list strings
					free_string = true;
				...
				GRRLIB_DrawImg(UI_SORT_X, 200, date_img, 0, 1, 1, LIGHT_BTN_COLOR);
			}

			if (sort_up_down == 0) GRRLIB_DrawImg(UI_SORT_ARROW_X, 220, sort_arrow_down_img, 0, 1, 1, LIGHT_BTN_COLOR);
			else if (sort_up_down == 1) GRRLIB_DrawImg(UI_SORT_ARROW_X, 204, sort_arrow_up_img, 0, 1, 1, LIGHT_BTN_COLOR);
			else if (sort_up_down == 6) GRRLIB_DrawImg(UI_SORT_ARROW_X, 170, sort_arrow_down_img, 0, 1, 1, LIGHT_BTN_COLOR);
			else if (sort_up_down == 7) GRRLIB_DrawImg(UI_SORT_ARROW_X, 154, sort_arrow_up_img, 0, 1, 1, LIGHT_BTN_COLOR);

			//Queue icon
			if (UI_isOnImg(ir, UI_SORT_X, 370, down_img)) {
				doRumble = true;
				GRRLIB_DrawImg(UI_SORT_X, 370, down_img, 0, 1, 1, LIGHT_BTN_COLOR_HOVER);
				if (setting_tool_tip == true) { UI_drawTooltip(UI_SORT_X, 370, STR_QUEUE); }
				...
				GRRLIB_DrawImg(UI_SORT_X, 370, down_img, 0, 1, 1, LIGHT_BTN_COLOR);
			}
		...
		if (UI_isOnImg(ir, 483, 16, sd_card_img)) {
			doRumble = true;
			GRRLIB_DrawImg(483, 16, sd_card_img, 0, 1, 1, BTN_COLOR_HOVER);
			...
			GRRLIB_DrawImg(483, 16, sd_card_img, 0, 1, 1, BTN_COLOR);
		}
		if (UI_isOnImg(ir, 546, 16, help_img)) {
			doRumble = true;
			GRRLIB_DrawImg(546, 16, help_img, 0, 1, 1, BTN_COLOR_HOVER);
			...
			GRRLIB_DrawImg(546, 16, help_img, 0, 1, 1, BTN_COLOR);
		}
		...
		if (updating >= 0 && strcmp(homebrew_list[0].name,"000") != 0) {
			...
			if ((updating < array_length (homebrew_list) && new_updating < array_length (homebrew_list)) || new_updating == 10000) {
				...
				// Cancel prompt
				if ((download_in_progress == true || extract_in_progress == true) && (cancel_download == true || cancel_extract == true) && cancel_confirmed == false) {
					if (cancel_wait == 0) {
						cancel_wait = 10;
					}
		...
		if (ACTIVITIES_current() == ACTIVITY_INFO) {
			...
			if (UI_isOnSquare(ir, 164, 146, 440, 44)) {
				doRumble = true;
		...
		if (ACTIVITIES_current() == ACTIVITY_HELP_QUEUE) {
			if (help_section == 0) {
				GRRLIB_DrawImg(UI_PAGE_X, 128, help_queue_img, 0, 1, 1, 0xFFFFFFFF);
				...
				if (UI_isOnImg(ir, 568, 135, next_img)) {
					doRumble = true;
					GRRLIB_DrawImg(568, 135, next_img, 0, 1, 1, BTN_COLOR_HOVER);
					if (pressed & WPAD_BUTTON_A || pressed & WPAD_BUTTON_2 || pressed_gc & PAD_BUTTON_A) {
						help_section = 1;
					}
				} else {
					GRRLIB_DrawImg(568, 135, next_img, 0, 1, 1, BTN_COLOR);
				}
				GRRLIB_DrawText(522, 418, STR_1_OF_3, FONTSIZE_SMALLER, TEXT_COLOR_SECONDARY);
			} else if (help_section == 1) {
				GRRLIB_DrawImg(UI_PAGE_X, 128, help_queue_2_img, 0, 1, 1, 0xFFFFFFFF);
				...
				if (UI_isOnImg(ir, 56, 135, prev_img)) {
					doRumble = true;
					GRRLIB_DrawImg(56, 135, prev_img, 0, 1, 1, BTN_COLOR_HOVER);
					if (pressed & WPAD_BUTTON_A || pressed & WPAD_BUTTON_2 || pressed_gc & PAD_BUTTON_A) {
						help_section = 0;
					}
				} else {
					GRRLIB_DrawImg(56, 135, prev_img, 0, 1, 1, BTN_COLOR);
				}
				if (UI_isOnImg(ir, 568, 135, next_img)) {
					doRumble = true;
					GRRLIB_DrawImg(568, 135, next_img, 0, 1, 1, BTN_COLOR_HOVER);
				...
				GRRLIB_DrawText(522, 418, STR_2_OF_3, FONTSIZE_SMALLER, TEXT_COLOR_SECONDARY);
			} else if (help_section == 2) {
				GRRLIB_DrawImg(UI_PAGE_X, 128, help_queue_3_img, 0, 1, 1, 0xFFFFFFFF);
				...
				GRRLIB_DrawText(522, 418, STR_3_OF_3, FONTSIZE_SMALLER, TEXT_COLOR_SECONDARY);
			}
		}
		...
		if (ACTIVITIES_current() == ACTIVITY_SETTINGS) {
			...
			if (menu_section == 0) {
				...
				if (setting_hide_installed == true) GRRLIB_DrawImg(498, 198, app_tick_img, 0, 1, 1, 0xFFFFFFFF);
				else GRRLIB_DrawImg(498, 198, setting_cross_img, 0, 1, 1, 0xFFFFFFFF);
				if (setting_rumble == true) GRRLIB_DrawImg(498, 248, app_tick_img, 0, 1, 1, 0xFFFFFFFF);
				else GRRLIB_DrawImg(498, 248, setting_cross_img, 0, 1, 1, 0xFFFFFFFF);
				if (setting_update_icon == true) GRRLIB_DrawImg(498, 298, app_tick_img, 0, 1, 1, 0xFFFFFFFF);
				else GRRLIB_DrawImg(498, 298, setting_cross_img, 0, 1, 1, 0xFFFFFFFF);
				if (setting_online == false) GRRLIB_DrawImg(498, 348, app_tick_img, 0, 1, 1, 0xFFFFFFFF);
				...
				if (setting_tool_tip == true) GRRLIB_DrawImg(498, 148, app_tick_img, 0, 1, 1, 0xFFFFFFFF);
				else GRRLIB_DrawImg(498, 148, setting_cross_img, 0, 1, 1, 0xFFFFFFFF);
				if (setting_use_sd == true) GRRLIB_DrawImg(498, 198, app_tick_img, 0, 1, 1, 0xFFFFFFFF);
				else GRRLIB_DrawImg(498, 198, setting_cross_img, 0, 1, 1, 0xFFFFFFFF);
				...
				if (setting_disusb == true) GRRLIB_DrawImg(498, 148, app_tick_img, 0, 1, 1, 0xFFFFFFFF);
				else GRRLIB_DrawImg(498, 148, setting_cross_img, 0, 1, 1, 0xFFFFFFFF);
				if (setting_wiiside == true) GRRLIB_DrawImg(498, 198, app_tick_img, 0, 1, 1, 0xFFFFFFFF);
				else GRRLIB_DrawImg(498, 198, setting_cross_img, 0, 1, 1, 0xFFFFFFFF);
				if (setting_server == true) GRRLIB_DrawImg(498, 248, app_tick_img, 0, 1, 1, 0xFFFFFFFF);
				else GRRLIB_DrawImg(498, 248, setting_cross_img, 0, 1, 1, 0xFFFFFFFF);
			...
			// Settings navigation
			if (menu_section > 0) {
				if (((pressed & WPAD_BUTTON_MINUS) || (setting_wiiside == false && pressed & WPAD_BUTTON_LEFT) || (pressed_gc & PAD_TRIGGER_L) || (pressed_gc & PAD_BUTTON_LEFT)) && select_repo == false && select_category == false && select_sort == false) {
					menu_section--;
				}
				if (UI_isOnImg(ir, 296, 400, prev_img) && select_repo == false && select_category == false && select_sort == false) {
					doRumble = true;
			...
			if (menu_section < 2) {
				if (((pressed & WPAD_BUTTON_PLUS) || (setting_wiiside == false && pressed & WPAD_BUTTON_RIGHT) || (pressed_gc & PAD_TRIGGER_R) || (pressed_gc & PAD_BUTTON_RIGHT)) && select_repo == false && select_category == false && select_sort == false) {
					menu_section++;
				}
				if (UI_isOnImg(ir, 452, 400, next_img) && select_repo == false && select_category == false && select_sort == false) {
					doRumble = true;
		...
		// Debugging
		if (false) {
			GRRLIB_Rectangle(0, 0, SCREEN_WIDTH, (SCREEN_HEIGHT - 420) / 2, 0x00FF0020, true);
			GRRLIB_Rectangle(0, SCREEN_HEIGHT - (SCREEN_HEIGHT - 420) / 2, SCREEN_WIDTH, (SCREEN_HEIGHT - 420) / 2, 0x00FF0020, true);
			GRRLIB_Circle(0, 0, 8, 0xFF0000FF, true);
			GRRLIB_Circle(0, SCREEN_HEIGHT, 8, 0xFF0000FF, true);
			GRRLIB_Circle(SCREEN_WIDTH, SCREEN_HEIGHT, 8, 0xFF0000FF, true);
			GRRLIB_Circle(SCREEN_WIDTH, 0, 8, 0xFF0000FF, true);
		}
